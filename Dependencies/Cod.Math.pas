...
interface
  uses
  SysUtils, Classes, Math, Cod.StringUtils, Cod.Types,
  Cod.ArrayHelpers;
  ...
  function GenerateRandomSequence(count: Integer): TIntArray;
...
const
    num_digit = ['0'..'9'];
    num_content = num_digit + ['.', ','];
    add_indic = ['+', '-'];
    multp_indic = ['*', '/'];
    oper_indic = add_indic + multp_indic;
...
begin
  ...
  for I := 1 to length(str) do
    if CharInSet(str[I], add_indic) then
      il := il + str[I]
    else
      begin
        if length(il) > 1 then
...
begin
  ...
  repeat
    ...
    PS := P + 1 + Pos(Copy(spar, P + 1, Length(spar)), InText);
    PE := P + 1 + Pos(Copy(epar, P + 1, Length(epar)), InText);
...
begin
    ...
    begin
      ...
      if S > 1 then
        begin
          ...
          if CharInSet(CA, num_content) then
            begin
  ...
  repeat
    // Operation type
    T := 0;
    C:= DEF_INIT + Pos('*', Copy(Ecuation, DEF_INIT, Length(Ecuation)));
    if C <> DEF_INIT then
      T := 1;
    if C = DEF_INIT then
      begin
    ...
    for I := C - 1 downto 1 do
        begin
          ...
          if CharInSet(CA, num_content) or ((I = 1) and (CA = '-')) then
            begin
              S := I;
