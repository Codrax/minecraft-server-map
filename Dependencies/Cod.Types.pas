...
interface
  uses
  SysUtils, Classes, Variants, Math, Types,
  DateUtils;
  ...
  type
    ...
    TRoundRect = record
        ...
        constructor Create(ATopLeft, ABottomRight: TPoint; Rnd: integer); overload;
        constructor Create(SRect: TRect; Rnd: integer); overload;
        constructor Create(ALeft, ATop, ARight, ABottom: integer; Rnd: integer); overload;
    end;
    ...
    public
      ...
      procedure SafeDelete(Index: Integer);

      function IndexOf(const Value: integer): integer;
      function Find(aValue : Integer; out Index: Integer): Boolean; virtual;
...
function GetValidRect(Points: TArray<TPoint>): TRect;
var
  I: Integer;
begin
...
constructor TRoundRect.Create(ATopLeft, ABottomRight: TPoint; Rnd: integer);
begin
  Rect := TRect.Create(ATopLeft, ABottomRight);
...
constructor TRoundRect.Create(ALeft, ATop, ARight, ABottom, Rnd: integer);
begin
  Rect := TRect.Create(ALeft, ATop, ARight, ABottom);
...
procedure TIntegerList.SafeDelete(Index: Integer);
begin
...
function TIntegerList.Find(aValue: Integer; out Index: Integer):
Boolean;
var
  L, H, I, C: Integer;
...
procedure TIntegerList.LoadFromString(AString, ASeparator: string);
var
  I: integer;
  AItems: TStringArray;
begin
  ...
  for I := 0 to System.High(AItems) do
    if AItems[I] <> '' then
      Add( strtoint(AItems[I]) );
end;

function TIntegerList.ToString(ASeparator: string): string;
var
  I, ACount: integer;
begin
  Result := '';
  ...
  for I := 0 to ACount-1 do
    Result := Concat(Result, Self[I].ToString, ASeparator);
